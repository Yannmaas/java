...
public class StockService 
{
    ...
    {
        int codeRequete = 0;
        int reqMaj = ConnexionMySql.execReqMaj("UPDATE STOCK SET QTESTOCK = " + unStock.getQteStock() + " WHERE MATRICULE ='" + unStock.getUnVisiteur().getMatricule() +"' AND MED_DEPOTLEGAL = '" + unStock.getUnMedicament().getMed_depotlegal() + "'");
        ConnexionMySql.fermerConnexionBd();
        if (reqMaj == 1)
        {
            codeRequete = 1;
            Stock stock = StockDao.rechercher(unStock.getUnVisiteur().getMatricule(), unStock.getUnMedicament().getMed_depotlegal());
            if (stock != null)
            {
                codeRequete = 2;
            }
